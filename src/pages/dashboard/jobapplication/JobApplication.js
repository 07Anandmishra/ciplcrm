...
const JobApplication = () => {
  ...
  const getAssigneeData = async () => {
    ...
    if (jobId !== "" || null || undefined) {
      ...
      const response = request.data;
      console.log("response line 175", response);
      if (response) {
          ...
          {jsonData?.data?.userPermissions?.find((x) =>
            ...
            <button
              onClick={() => {
                ...
                // setSubQualificationId(row?.subqualifications[1]?.subqualifications_id);

                // setSubQualificationId(row?.subqualifications?.id)
                // setSubQualificationId(row?.subqualification_id?.id)
